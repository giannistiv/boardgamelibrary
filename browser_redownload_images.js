// Run this in the browser console while on boardgamegeek.com (logged in)
// Uses api.geekdo.com/api/images to get full box art URLs (not cropped OG banners)
// Auto-downloads bgg_image_urls.json when done — paste contents back to Claude
...
(async function() {
  ...
  var results = {};
  var failed = [];
  console.log("Fetching full box art URLs for " + ids.length + " games via geekdo images API...");

  for (var i = 0; i < ids.length; i++) {
    var id = ids[i];
    try {
      var r = await fetch("https://api.geekdo.com/api/images?ajax=1&objectid=" + id + "&objecttype=thing&nosession=1&showcount=1&size=original&sort=hot");
      if (r.ok) {
        var data = await r.json();
        if (data && data.images && data.images.length > 0) {
          var url = data.images[0].imageurl_lg;
